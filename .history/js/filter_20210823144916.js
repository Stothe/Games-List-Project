...
function filterControls(){
    ...
    for (let i = 0; i < filterProperties.length; i++) {
        ...
        if(games2.length === 0){
            ...
            console.log(game[filterProperties[i]]);
        } else {
            games2 = games2 + games.filter(game => controlArray.includes(game[filterProperties[i]]));
            console.log(game[filterProperties[i]]);
        }
...
/*
const controlArray = [];
    const controlFilters = document.querySelectorAll(className);
    // controlFilters = controlFilters.find
    controlFilters.forEach(element => {
        if(element.checked){
            controlArray.push(element.name);
    ...
    console.log(controlArray);
    games2 = games.filter(game => controlArray.includes(game.control));
    console.log(games2);
