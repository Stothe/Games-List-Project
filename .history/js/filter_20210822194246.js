...
function filterControls(){
    for (let index = 0; index < filterProperties.length; index++) {
        ...
        const controlArray = [];
        const controlFilters = document.querySelectorAll("." + key);
        ...
        if(games2.length === 0){
            games2 = games.filter(game => controlArray.includes(game[key]));
        } else {
            games2 = games2 + games.filter(game => controlArray.includes(game[key]));
        }
